...
class ToolsViewModel @Inject constructor(
    ...
    fun onAddToolSelected(name:String, project:String, description:String){
        if (name.isNotEmpty() && project.isNotEmpty()){
            if(description.isNotEmpty()) addTool.postValue(addTolCaseUse(name,project, description))
            else addTool.postValue(false)
        }else{
    ...
    fun onSearchToolSelected(name:String) {
        if(name.isNotEmpty()){
            viewModelScope.launch {
                searchTool.postValue(withContext(Dispatchers.IO) {searchToolUseCase(name)})
            }
        }else{
